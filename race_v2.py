...
class Game:
    def __init__(self):
        ...
        while (self.stone_startx >= self.obstacle_startx and self.stone_startx
               <= self.obstacle_startx + self.obstacle_width) or (
                   self.stone_startx + self.stone_width >= self.obstacle_startx
                   and self.stone_startx + self.stone_width <=
                   self.obstacle_startx + self.obstacle_width):
            self.stone_startx = random.randrange(0, display_width)
    ...
    def things_dodged(self, count):
        ...
        info = pygame.font.Font("freesansbold.ttf",
                                20).render("Info: i", True, white)
        SCREEN.blit(info, (50, 50))

    # Draw the information key on the left top
    def draw_info(self):
        info = pygame.font.Font("freesansbold.ttf",
                                20).render("Pause: p", True, white)
        SCREEN.blit(info, (50, 75))
        info = pygame.font.Font("freesansbold.ttf",
                                20).render("Quit: q", True, white)
        SCREEN.blit(info, (50, 100))
        info = pygame.font.Font("freesansbold.ttf",
                                20).render("Clean High: c", True, white)
        SCREEN.blit(info, (50, 125))
        info = pygame.font.Font("freesansbold.ttf",
                                20).render("Skip to End: spacebar", True,
                                           white)
    ...
    def _touch_stone(self):
        if self.playery < self.stone_starty + self.stone_height:
            if (self.playerx > self.stone_startx and self.playerx <
                (self.stone_startx + self.stone_width)) or (
                    self.playerx + car_width > self.stone_startx
                    and self.playerx + car_width <
                    (self.stone_startx + self.stone_width)):
                self.draw_stone_flag = False
                return True
        return False
    ...
    def frame(self):
        ...
        if self.obstacle_starty > display_height:
            ...
            self.obstacle_startx = random.randrange(
                0, display_width - self.obstacle_width)
            self.score += 1
        ...
        if not self.draw_stone_flag:  # if not able to draw_stone, move stone back up
            ...
            while (
                    self.stone_startx >= self.obstacle_startx
                    and self.stone_startx <=
                    self.obstacle_startx + self.obstacle_width
            ) or (self.stone_startx + self.stone_width >= self.obstacle_startx
                  and self.stone_startx + self.stone_width <=
                  self.obstacle_startx + self.obstacle_width):
                self.stone_startx = random.randrange(0, display_width)
        if self.draw_stone_flag and not self._touch_stone(
        ):  # start drawing stones, but not at touching moment
            # if self.stone_num in self.stone_collected:
            #     self.stone_num +=1
            self.draw_stone(self.stone_startx, self.stone_starty,
                            self.stone_num)
